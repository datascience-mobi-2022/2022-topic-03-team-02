...
First of all, use `install.packages()` to install the packages listed below!!
...
In the second normalization step, we want to set every protein's amount within one replicate to be 100 per condition. For example, the protein represented by row 1 has a sum over all 25 fractions of 100 in Ctrl_Rep1. To archive this, we begin with building 6 smaller data frames for every replicate of each condition.
...
Now, we fit the function from above so it identifies the global maxima of the 6 replicates for every protein.
```{r}
...
Last, we can apply the function on our 6 data sets (6 replicates) in order to find all global maxima and combine them in `df_global_maxima_rep`.
```{r}
...
# save all global maxima of every replicate in one df
...
Here, we are going to perform the same analysis as above (significant difference of protein amount at the global maximum (combined)), but with the not combined data. This means, we are using the normalized but not combined data for global maxima as well as protein amounts. Above, we used the normalized and combined data for global maxima. 
```{r}
pval_rep <- c()
...
As stated above, R-DeePs must meet both criteria. Therefore, we can identify R-DeePs in our dataset by computing the intersect between proteins with significant shift and proteins with significant difference in protein amount. Here, we computed this intersect once for the results using combined data for global maxima and once using the not combined data.
```{r}
# combined values
...
We decided to use the R-DeePs computed with replicates values only as our results (657).
```{r}
...
```{r}
alpha1 <- ifelse(df_umap$shift == "no shift", 0.1, 1)
...
```{r}
alpha2 <- ifelse(df_umap$rdeep == "non rdeep", 0.1, 1)

ggplot(df_umap, aes(x = V1, y = V2, color = rdeep)) +
  geom_point(alpha = alpha2) +
  ggtitle("UMAP of normalized and combined data")
```
...
```{r}
# number of our RDeePs which are non RBP
...
```{r}
# number of our non RDeePs which are RBP
...
```{r}
# number of our non RDeePs which are non RBP
...
```{r}
# number of our RDeePs which are RDeeP

counter <- 0

for (i in 1:nrow(HS_RBPs)) {
  if (HS_RBPs$Entry_Name.x[i] %in% rdeep) {
    if (HS_RBPs$R.DeeP_HeLa[i] == "X") {
      counter <- counter + 1
    }
  }
}
...
```{r}
# number of our non RDeePs which are RDeeP

counter <- 0

for (i in 1:nrow(HS_RBPs)) {
  if (HS_RBPs$Entry_Name.x[i] %in% non_rdeep) {
    if (HS_RBPs$R.DeeP_HeLa[i] == "X") {
      counter <- counter + 1
    }
  }
...
There is a difference visible when considering local maxima as well in the way we did above.
...
## plots selection criteria

```{r}
df_plot_ctrl <- as.data.frame(t(df_normalized_combined)[1:25, ])
df_plot_ctrl$fraction <- 1:25

df_plot_rnase <- as.data.frame(t(df_normalized_combined)[26:50, ])
df_plot_rnase$fraction <- 1:25
```

```{r}
plot1 <- ggplot(df_plot_ctrl, aes(x = fraction, y = RS12_HUMAN)) +
  geom_line() +
  geom_point() +
  geom_vline(xintercept = 17, color = "blue") +
  geom_vline(xintercept = 5, color = "red") +
  geom_hline(yintercept = df_plot_rnase$RS12_HUMAN[17], color = "red", linetype = "dotted") +
  geom_hline(yintercept = df_plot_ctrl$RS12_HUMAN[17], color = "blue", linetype = "dotted") +
  ggtitle("R-DeeP Ctrl (RS12_HUMAN)") +
  ylab("Protein amount") + xlab("Fraction")

plot2 <- ggplot(df_plot_rnase, aes(x = fraction, y = RS12_HUMAN)) +
  geom_line() +
  geom_point() +
  geom_vline(xintercept = 17, color = "blue") +
  geom_vline(xintercept = 5, color = "red") +
  ...
  geom_hline(yintercept = df_plot_ctrl$RS12_HUMAN[17], color = "blue", linetype = "dotted") +
  ggtitle("R-DeeP Ctrl (RS12_HUMAN)") +
  ggtitle("R-Deep RNase (RS12_HUMAN)") +
  ylab("Protein amount") + xlab("Fraction")
...
```{r}
plot1 <- ggplot(df_plot_ctrl, aes(x = fraction, y = CGL_HUMAN)) +
  geom_line() +
  geom_point() +
  geom_vline(xintercept = 8, color = "blue") +
  geom_vline(xintercept = 8, color = "blue") +
  geom_hline(yintercept = max(df_plot_rnase$CGL_HUMAN), color = "red", linetype = "dotted") +
  geom_hline(yintercept = max(df_plot_ctrl$CGL_HUMAN), color = "blue", linetype = "dotted") +
  ggtitle("non R-DeeP Ctrl (CGL_HUMAN)") +
  ylab("Protein amount") + xlab("Fraction")

plot2 <- ggplot(df_plot_rnase, aes(x = fraction, y = CGL_HUMAN)) +
  geom_line() +
  geom_point() +
  ...
  geom_hline(yintercept = max(df_plot_rnase$CGL_HUMAN), color = "red", linetype = "dotted") +
  geom_hline(yintercept = max(df_plot_ctrl$CGL_HUMAN), color = "blue", linetype = "dotted") +
  ggtitle("non R-Deep RNase (CGL_HUMAN)") +
  ylab("Protein amount") + xlab("Fraction")
