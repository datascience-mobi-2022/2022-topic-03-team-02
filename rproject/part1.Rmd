...
Now, we can check how many and which values are below a threshold t.
```{r}
t <- 0.6  # threshold value 

cor_mat <- cbind(c1c2, c1c3, c2c3, r1r2, r1r3, r2r3)
sum(cor_mat < t)
sum(c1c2 < t)
sum(c1c3 < t)
sum(c2c3 < t)
...
```{r}
tc <- c()
# tc will contain the proteinnames of all correlation values < t
...
```{r}
# length of unique elements of tc will give us the number of proteins which will be deleted under t
length(unique(tc))
```
Seems like 36 proteins have replicates that do not correlate well with each other.

We can also have a look at the correlation values we just computed by visualizing them with a plot. For example, we can have a look at the distribution:
