...
for (i in 1:nrow(df_global_maxima)) {
  ...
  if (x$p.value < 0.05) {
    sig_dif_proteins <-
      append(sig_dif_proteins, rownames(df_global_maxima)[i])
  }
}
...
```{r}
y <- which(pval_adjust < 0.05)
sig_dif_proteins2 <- rownames(df_global_maxima)[y]
```
...
# with fdr correction
rdeep2 <- c(intersect(sig_dif_proteins2, shift_proteins))
length(rdeep2)
```


Cluster Idee: UMAP Clustern der Proteine und dann V1 und V2 gegeneinander plotten und Proteine färben, z.B. nach no shift, links shift, right shift
